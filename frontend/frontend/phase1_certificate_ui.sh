...
echo "🚀 Starting Phase 1: Certificate UI Frontend..."

cd /opt/swaeduae/frontend

# 1. Create AdminSendCertificate.jsx
...
const AdminSendCertificate = () => {
  ...
  const handleSend = async () => {
    try {
      const res = await fetch('http://localhost:5000/certificates/send', {
        method: 'POST',
...
# 2. Create VerifyCertificate.jsx
...
sed -i '/react-router-dom/a import AdminSendCertificate from "./pages/Certificate/AdminSendCertificate";\nimport VerifyCertificate from "./pages/Certificate/VerifyCertificate";' src/App.jsx

sed -i '/<Routes>/a \
        <Route path="/admin/send-certificate" element={<AdminSendCertificate />} />\
        <Route path="/verify-certificate" element={<VerifyCertificate />} />' src/App.jsx

# 4. Git commit and push
echo "📤 Committing changes..."
git add .
git commit -m "📄 Phase 1: Add certificate send + verify UI"
git push origin main

echo "✅ Certificate UI Frontend complete."
